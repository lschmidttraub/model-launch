...
from utils import nanoid, extract_model_name, setup_logging, generate_job_script, run_job, fetch_bootstrap_addresses
...
def parse_args():
    ...
    parser.add_argument("--slurm-nodes", type=int, required=True, help="Number of nodes to use from the selected het-group")
    ...
    parser.add_argument("--hetgroup", type=int, required=True, help="Hetjob group")
...
def main():
    ...
    template_path = os.path.join(script_dir, "template_existing_alloc.jinja")
    ...
    with tempfile.NamedTemporaryFile(mode="w", suffix=".sh", delete=False) as temp_file:
        ...
        run_job(temp_file.name)

        job_id = os.environ.get("SLURM_JOB_ID", "<current-job>")
        log_dir = f"logs/{job_id}"

        logging.info("")
        logging.info(f"Serving step logs will be written under: {log_dir}/")
        logging.info("")
        ...
        logging.info("")
        logging.info("To cancel the outer job:")
